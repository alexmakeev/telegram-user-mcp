...
# SSE keepalive: inject ping=30 into EventSourceResponse used by mcp.server.sse
# so the SSE connection does not die on idle timeout (proxy/load balancer cuts
# connections after ~5 minutes of silence without a heartbeat).
import mcp.server.sse as _mcp_sse_module
from sse_starlette import EventSourceResponse as _OriginalEventSourceResponse

class _PingEventSourceResponse(_OriginalEventSourceResponse):
    """EventSourceResponse that always sends a SSE comment keepalive every 30 s."""

    def __init__(self, *args, **kwargs):
        kwargs.setdefault("ping", 30)
        super().__init__(*args, **kwargs)

_mcp_sse_module.EventSourceResponse = _PingEventSourceResponse  # type: ignore[attr-defined]
...
# Deploy guard: prevent unauthorized container starts
# This variable is ONLY set in Dokploy production environment.
# If missing, the container was started manually (docker-compose up) — refuse to run.
if os.getenv("DEPLOY_CONFIRMED_BY_ADMIN") != "yes":
    print("", file=sys.stderr)
    print("=" * 66, file=sys.stderr)
    print("  DEPLOY GUARD: unauthorized container start blocked", file=sys.stderr)
    print("=" * 66, file=sys.stderr)
    print("", file=sys.stderr)
    print("  This container must be deployed via Dokploy by an admin.", file=sys.stderr)
    print("  Do NOT start it with 'docker-compose up' or 'docker run'.", file=sys.stderr)
    print("", file=sys.stderr)
    print("  Why: Telegram session files are sensitive. Running multiple", file=sys.stderr)
    print("  instances or unauthorized copies can invalidate the session", file=sys.stderr)
    print("  and lock the account.", file=sys.stderr)
    print("", file=sys.stderr)
    print("  Contact: @alexmakeev87 (Telegram admin)", file=sys.stderr)
    print("=" * 66, file=sys.stderr)
    print("", file=sys.stderr)
    sys.exit(1)
...
TELEGRAM_SESSION_NAME = os.getenv("TELEGRAM_SESSION_NAME", "telegram_user")
TELEGRAM_SESSION_DIR = os.getenv("TELEGRAM_SESSION_DIR", "/app/sessions")
...
# Session priority: file-based (persistent) > StringSession (stateless)
# File-based session survives DC migrations and container restarts
_session_file = os.path.join(TELEGRAM_SESSION_DIR, TELEGRAM_SESSION_NAME)
if os.path.exists(f"{_session_file}.session"):
    logger.info(f"Using file-based session: {_session_file}.session")
    _session = _session_file
    _session_type = "file"
elif SESSION_STRING and not SESSION_STRING.startswith("HONEYPOT"):
    logger.info("Using StringSession from env (no file-based session found)")
    logger.warning("StringSession is stateless — DC migrations will be lost on restart")
    _session = StringSession(SESSION_STRING)
    _session_type = "string"
else:
    # First run — will create file-based session during QR auth
    logger.info(f"No session found, will create file-based session at: {_session_file}")
    os.makedirs(TELEGRAM_SESSION_DIR, exist_ok=True)
    _session = _session_file
    _session_type = "file-new"
...
async def _on_raw_update(event):
    """Log raw updates that indicate connection state changes."""
    event_name = type(event).__name__
    if 'Config' in event_name or 'DcOption' in event_name:
        logger.info(f"Telegram config update: {event_name}")
...
def log_tool_call(tool_name: str, **kwargs):
    ...
    params = ', '.join(f'{k}={repr(v)[:100]}' for k, v in kwargs.items() if v is not None)
...
async def get_chats(page: int = 1, page_size: int = 20) -> str:
    ...
    """
    log_tool_call("get_chats", page=page, page_size=page_size)
    try:
...
async def get_messages(chat_id: Union[int, str], page: int = 1, page_size: int = 20) -> str:
    ...
    """
    log_tool_call("get_messages", chat_id=chat_id, page=page, page_size=page_size)
    try:
...
async def warm_topic_cache(entity, topic_id: int) -> bool:
    """Warm Telethon cache for forum topics so reply_to works.

    Telethon requires the topic root service message to be in its local cache
    in order to correctly resolve reply_to for forum topics. User-created topics
    can usually be fetched by message ID, but bot-created topics cannot — their
    service message is not returned by get_messages(ids=[...]). In that case we
    fall back to GetForumTopicsRequest which populates the cache with all topic
    metadata, including service messages for bot-created topics.
    """
    try:
        # Fast path: works for user-created topics
    ...
    try:
        # Fallback: fetch full forum topic list (works for bot-created topics)
        input_peer = await client.get_input_entity(entity)
        await client(GetForumTopicsRequest(
            peer=input_peer,
            offset_date=None,
            offset_id=0,
            offset_topic=0,
...
@validate_id("chat_id")
async def send_message(chat_id: Union[int, str], message: str, topic_id: Optional[int] = None, schedule_minutes: Optional[int] = None) -> str:
    """
    Send a message to a specific chat.
    Args:
        chat_id: The ID or username of the chat.
        message: The message content to send.
        topic_id: Optional forum topic ID. When provided, the message is sent to that forum topic (sets reply_to to the topic root message ID, which is how Telethon targets forum topics).
        schedule_minutes: Optional number of minutes from now to schedule the message. When provided, the message is scheduled for delivery after that many minutes instead of being sent immediately.
    """
    log_tool_call("send_message", chat_id=chat_id, topic_id=topic_id, schedule_minutes=schedule_minutes)
    try:
        entity = await client.get_entity(chat_id)
        kwargs = {}
        if topic_id is not None:
            # Warm Telethon cache so reply_to resolves correctly for both
            # user-created and bot-created forum topics.
            await warm_topic_cache(entity, topic_id)
            kwargs['reply_to'] = topic_id
        if schedule_minutes is not None:
            kwargs['schedule'] = timedelta(minutes=schedule_minutes)
        await client.send_message(entity, message, **kwargs)
        if schedule_minutes is not None:
            return f"Message scheduled to be sent in {schedule_minutes} minute(s)."
...
async def list_topics(
    ...
    """
    log_tool_call("list_topics", chat_id=chat_id, limit=limit, search_query=search_query)
    try:
        entity = await client.get_entity(chat_id)
...
@validate_id("chat_id")
async def send_file(chat_id: Union[int, str], file_path: str, caption: str = None, topic_id: Optional[int] = None) -> str:
    """
    ...
    Args:
        ...
        topic_id: Optional forum topic ID. When provided, the file is sent to that forum topic (sets reply_to to the topic root message ID, which is how Telethon targets forum topics).
    """
    log_tool_call("send_file", chat_id=chat_id, file_path=file_path, caption=caption, topic_id=topic_id)
    try:
        ...
        if topic_id is not None:
            await warm_topic_cache(entity, topic_id)
            kwargs['reply_to'] = topic_id
        await client.send_file(entity, file_path, caption=caption, **kwargs)
...
@validate_id("chat_id")
async def send_voice(chat_id: Union[int, str], file_path: str, topic_id: Optional[int] = None) -> str:
    """
    ...
    Args:
        ...
        topic_id: Optional forum topic ID. When provided, the voice is sent to that forum topic.
    """
    try:
        if not os.path.isfile(file_path):
            return f"File not found: {file_path}"
        ...
        if topic_id is not None:
            await warm_topic_cache(entity, topic_id)
            kwargs['reply_to'] = topic_id
        await client.send_file(entity, file_path, voice_note=True, **kwargs)
...
async def reply_to_message(chat_id: Union[int, str], message_id: int, text: str) -> str:
    ...
    try:
        entity = await client.get_entity(chat_id)
        # Warm Telethon cache for forum topics (bot-created topics may not be in cache).
        # Fetching the message by ID or any message in the topic forces cache population,
        # allowing reply_to to correctly target the topic instead of falling back to General.
        try:
            msgs = await client.get_messages(entity, ids=[message_id])
            if not msgs or not msgs[0]:
                await client.get_messages(entity, reply_to=message_id, limit=1)
        except Exception:
            pass  # Cache warm-up failed; still attempt send
...
async def search_messages(chat_id: Union[int, str], query: str, limit: int = 20) -> str:
    ...
    """
    log_tool_call("search_messages", chat_id=chat_id, query=query, limit=limit)
    try:
        entity = await client.get_entity(chat_id)
...
def _check_port_free(port: int = 8000) -> bool:
    """Check if port is free before starting. Prevents dual-instance conflicts."""
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            s.settimeout(1)
            result = s.connect_ex(('127.0.0.1', port))
            if result == 0:
                print(f"FATAL: Port {port} is already in use. Another instance may be running.", file=sys.stderr)
                return False
            return True
    except Exception:
        return True
...
def _acquire_lock():
    """Acquire exclusive file lock to prevent duplicate instances."""
    ...
    try:
        ...
        print("FATAL: Another instance is already running (lock held).", file=sys.stderr)
        sys.exit(0)  # exit 0 = don't restart
    except Exception as e:
        print(f"WARNING: Could not acquire lock: {e}", file=sys.stderr)
        return None


async def _telegram_keepalive(client: TelegramClient, interval: int = 300) -> None:
    """Periodic ping to keep Telegram TCP connection alive and detect DC migrations."""
    last_dc = getattr(client.session, 'dc_id', None)
    logger.info(f"Keepalive started: interval={interval}s, initial DC={last_dc}")

    while True:
        await asyncio.sleep(interval)
        try:
            me = await client.get_me()
            current_dc = getattr(client.session, 'dc_id', None)

            if current_dc != last_dc:
                logger.warning(f"DC migration detected: DC {last_dc} -> DC {current_dc}")
                last_dc = current_dc
                # File-based session saves this automatically
                # Log session file state for debugging
                if hasattr(client.session, 'filename') and client.session.filename:
                    try:
                        mtime = os.path.getmtime(client.session.filename)
                        logger.info(f"Session file updated: {client.session.filename}, mtime={time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(mtime))}")
                    except OSError:
                        pass
            else:
                logger.debug(f"Keepalive OK: user={me.username}, DC={current_dc}")

        except ConnectionError as e:
            logger.warning(f"Keepalive: connection lost ({e}), attempting reconnect...")
            try:
                await client.connect()
                current_dc = getattr(client.session, 'dc_id', None)
                logger.info(f"Keepalive: reconnected successfully, DC={current_dc}")
                last_dc = current_dc
            except Exception as reconnect_err:
                logger.warning(f"Keepalive: reconnect failed ({reconnect_err}), will retry in {interval}s")
        except Exception as e:
            logger.warning(f"Keepalive: unexpected error ({type(e).__name__}: {e}), continuing")


async def _main() -> None:
    lockfile = None
    session_lockfile = None
    ready_file = "/tmp/mcp-ready"

    try:
        # Guard 1: Acquire exclusive instance lock
        lockfile = _acquire_lock()

        # Guard 2: Lock session file to prevent dual Telegram connections
        session_lock_path = f"{_session_file}.lock"
        os.makedirs(os.path.dirname(session_lock_path), exist_ok=True)
        session_lockfile = open(session_lock_path, 'w')
        try:
            fcntl.flock(session_lockfile, fcntl.LOCK_EX | fcntl.LOCK_NB)
            logger.info(f"Session lock acquired: {session_lock_path}")
        except BlockingIOError:
            logger.error(f"Session file is locked by another process: {session_lock_path}")
            logger.error("Cannot start: another instance is using the same Telegram session")
            sys.exit(2)

        # Guard 3: Check port is free (SSE mode only)
        transport = os.getenv("TRANSPORT", "stdio")
        if transport == "sse" and not _check_port_free(8000):
            sys.exit(0)  # exit 0 = permanent error, don't restart

        # Guard 4: Register signal handlers for graceful shutdown
        ...
        logger.info(f"Connected to Telegram: DC={dc_id}, session_type={_session_type}")

        # Log session file info
        if hasattr(client.session, 'filename') and client.session.filename:
            session_path = client.session.filename
            if os.path.exists(session_path):
                mtime = os.path.getmtime(session_path)
                size = os.path.getsize(session_path)
                logger.info(f"Session file: {session_path}, size={size}B, modified={time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(mtime))}")

        if not await client.is_user_authorized():
            logger.error(f"Not authorized: DC={dc_id}, session_type={_session_type}")
            print("ERROR: Telegram session is not authorized. Re-run interactively to login.", file=sys.stderr)
            sys.exit(0)  # exit 0 = don't restart (permanent auth issue)
        logger.info("Telegram client connected. Starting MCP server...")

        # Start background keepalive to prevent idle TCP disconnects
        asyncio.create_task(_telegram_keepalive(client))
        logger.info("Telegram keepalive task started (interval=300s)")

        # Mark as ready (for healthcheck)
        ...
        error_msg = str(e)
        print(f"Error starting client: {error_msg}", file=sys.stderr)

        # AuthKeyDuplicatedError = permanent, don't restart
        if "AuthKeyDuplicated" in error_msg or "authorization key" in error_msg.lower():
            print("FATAL: Session invalidated. Generate a new session.", file=sys.stderr)
            sys.exit(0)  # exit 0 = don't restart via on-failure policy

        sys.exit(1)  # exit 1 = temporary error, allow restart
        ...
        if session_lockfile:
            try:
                session_lockfile.close()
                os.unlink(f"{_session_file}.lock")
            except OSError:
                pass
        if lockfile:
            try:
                lockfile.close()
                os.unlink("/tmp/telegram-mcp.lock")
            except Exception:
                pass
        # Ensure Telegram is disconnected
        if client.is_connected():
            await client.disconnect()
